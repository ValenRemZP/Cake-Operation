...
$userProfileData = fetch('SELECT * FROM userprofile WHERE id = ?', [
    'type' => 'i',
    'value' => $userid,
]);

$theme = $userProfileData ? THEME_MAPPING[$userProfileData['theme']] : THEME_MAPPING['default'];
...
if ($success) {
    ...
    <div class="alert alert-success w-fit grid-flow-col md:pr-6">
        ...
        <span>' . (SUCCESS_MAPPING[$success] ?? 'It worked!') . '</span>
    </div>
    <script>
        setTimeout(function() {
            var alerts = document.querySelectorAll(".alert");
...
<head>
    ...
    <title>Cake aboard! - <?php echo $route['title']; ?></title>
</head>
